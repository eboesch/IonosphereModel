...
if __name__ == "__main__":
    ...
    metrics = pd.Series(
        {"MSE": np.abs((targets - preds) ** 2).mean().item(), "MAE": np.abs(targets - preds).mean().item()}
    )
    print(metrics)
    ...
    if inferences_config["save_inferences"]:
        ...
        # for debug
        out_df.to_csv(
            "outputs/" + model_id + inferences_config["evaluation_data"] + str(year) + str(doy) + str(n) + ".csv",
            index=False,
        )
