...
def get_daily_solar_indices(year: int, doy: int, datapath: str):
    ...
    solar_indices = (
        row[["kp_index", "r_index", "dst_index", "f_index"]].to_numpy().reshape(-1)
    )  # extract the desired indices and convert to a numpy array
...
def get_hourly_solar_indices(year: int, doy: int, datapath: str):
    ...
    if doy == last_doy[year]:
        ...
        df_next = df.loc[
            (df["year"] == year) & (df["doy"] == doy + 1) & (df["hour"] == 0)
        ]  # get midnight of the next day
    df_next["hour"] = 24
    df_next["r_index"] = df_filtered["r_index"].iloc[
        -1
    ]  # r_index and f_index are the same all day -> keep the values of the current day
    ...
    df_filtered = df_filtered.drop(
        columns=["year", "doy"]
    ).to_numpy()  # extract the desired indices and convert to a numpy array
    return df_filtered
            ...
            for datapath in tqdm(datapaths):
                ...
                try:
                    ...
                    if subsampling_ratio == 1:
                        ...
                        seconds = [
                            int(30 / subsampling_ratio) * i + 30 * np.random.randint(0, 20)
                            for i in range(int(2880 * subsampling_ratio))
                        ]
...
if __name__ == "__main__":
    ...
    assert solar_indices_mode in [
        "none",
        "daily",
        "hourly",
    ], "solar_indices_mode must be one of 'none', 'daily' or 'hourly'."
            ...
            if months_in_year != "all":
                ...
                for month in months_in_year:
                    datapaths_per_month.append(
                        [
                            datapath + f"{year}/{str(doy + 1).zfill(3)}/ccl_{year}{str(doy + 1).zfill(3)}_30_5.h5"
                            for doy in range(month_indices[month - 1], month_indices[month])
                        ]
                    )
            else:
                ...
                for last_index, index in zip([0] + month_indices[:-1], month_indices):
                    datapaths_per_month.append(
                        [
                            datapath + f"{year}/{str(doy + 1).zfill(3)}/ccl_{year}{str(doy + 1).zfill(3)}_30_5.h5"
                            for doy in range(last_index, index)
                        ]
                    )
