...
def generate_split_train_val_test(data: pd.DataFrame, splits_path: str, train_frac=0.8) -> None:
    ...
    stations_df = (
        data[["station", "lat_sta", "lon_sta"]].groupby("station").agg("first").sort_values(by="station").reset_index()
    )
    ...
    splits_dict = {
        station: 0.5 * (station in val_stations) + (station in test_stations) for i, station in enumerate(stations)
    }
