...
def get_expected_rows_month(group, subsampling_ratio):
    """
...
def get_expected_rows_day(group, subsampling_ratio):
    """
    Returns the expected number of rows of the table, based on the subsampling ratio and group (train, test or val)
    """
    rows_per_raw_day = 20e6
...
def get_hourly_solar_indices(year, doy, datapath):
    ...
    df_filtered = df_filtered.drop(columns=['year', 'doy']).to_numpy()
    return df_filtered
...
def reorganize_day(year, doy, datapaths, output_path, subsampling_ratio, solar_indices_mode, solar_indices_path):
    """
    Subsamples the data of the corresponding month and year at the given datapaths and saves it at the indicated location.
    """
    for group in ["train", "val", "test"]:
        ...
        if os.path.exists(doy_output_path):
